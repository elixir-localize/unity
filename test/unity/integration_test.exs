defmodule Unity.IntegrationTest do
  ...
  describe "format/2" do
    test "formats a unit" do
      result = Unity.eval!("3 meters")
      assert {:ok, formatted} = Unity.format(result)
      assert formatted =~ "meter"
    end

    test "formats a number" do
      result = Unity.eval!("3 + 4")
      assert {:ok, "7"} = Unity.format(result)
    end

    test "verbose format" do
      result = Unity.eval!("3 meters to feet")
      assert {:ok, formatted} = Unity.format(result, format: :verbose, input: "3 meters to feet")
    ...
    test "terse format" do
      result = Unity.eval!("3 meters to feet")
      assert {:ok, formatted} = Unity.format(result, format: :terse)
      assert formatted =~ "9"
  ...
  describe "full expression pipeline examples from plan" do
    ...
    test "60 mph to km/h" do
      {:ok, result, _env} = Unity.eval("60 mph to km/h")
  ...
  describe "juxtaposition multiplication" do
    test "kg m / s^2 as force" do
      {:ok, result, _env} = Unity.eval("100 kg m / s^2")
      assert result.name == "kilogram-meter-per-square-second"
      assert_in_delta result.value, 100, 0.001
    end

    test "(3 + 4) m evaluates" do
      {:ok, result, _env} = Unity.eval("(3 + 4) m")
      assert result.name == "meter"
  ...
  describe "variables and environment chaining" do
    test "let binding and reuse" do
      {:ok, _result, env} = Unity.eval("let distance = 42.195 km")
      assert %Localize.Unit{name: "kilometer"} = env["distance"]
    end

    test "let and reference across evals" do
      {:ok, _, env} = Unity.eval("let distance = 42.195 km")
      {:ok, _, env} = Unity.eval("let time = 2 hours", env)
      {:ok, result, _env} = Unity.eval("distance / time", env)
      assert result.name == "kilometer-per-hour"
      assert_in_delta result.value, 21.0975, 0.001
    end

    test "underscore as previous result" do
      {:ok, result, env} = Unity.eval("10 meters")
      env = Map.put(env, "_", result)
      {:ok, result2, _env} = Unity.eval("_ to cm", env)
  ...
  describe "measurement system conversion" do
    test "100 meter to us" do
      {:ok, result, _env} = Unity.eval("100 meter to us")
    ...
    test "100 celsius to metric stays celsius" do
      {:ok, result, _env} = Unity.eval("100 celsius to metric")
      assert result.name == "celsius"
    end

    test "preferred with German locale selects metric" do
      Localize.put_locale(:de)
      {:ok, result, _env} = Unity.eval("100 fahrenheit to preferred")
      assert result.name == "celsius"
    ...
    test "preferred with English locale selects US" do
      Localize.put_locale(:en)
      {:ok, result, _env} = Unity.eval("100 meter to preferred")
      assert result.name in ["foot", "mile", "yard"]
    end

    test "imperial alias works same as uk" do
      {:ok, result_imp, _} = Unity.eval("100 meter to imperial")
      {:ok, result_uk, _} = Unity.eval("100 meter to uk")
    ...
    test "SI alias works same as metric" do
      {:ok, result_si, _} = Unity.eval("100 meter to SI")
      {:ok, result_metric, _} = Unity.eval("100 meter to metric")
  ...
  describe "mixed-unit display" do
    test "3.756 hours to h;min;s" do
      {:ok, result, _env} = Unity.eval("3.756 hours to h;min;s")
    ...
    test "1.5 hours to h;min" do
      {:ok, result, _env} = Unity.eval("1.5 hours to h;min")
  ...
  describe "locale-aware formatting" do
    test "German locale" do
      result = Unity.eval!("1234.5 meter to kilometer")
      Localize.put_locale(:de)
      {:ok, formatted} = Unity.format(result)
    ...
    test "format with explicit locale option" do
      result = Unity.eval!("1234.5 meter to kilometer")
      {:ok, formatted} = Unity.format(result, locale: :de)
      assert formatted =~ ","
    end
  end
  ...
  describe "double-star exponentiation" do
    test "s**2 evaluates same as s^2" do
      {:ok, result, _env} = Unity.eval("9 m**2")
  ...
  describe "formatter options" do
    test "digits option controls precision" do
      result = Unity.eval!("3 meters to feet")
      {:ok, formatted} = Unity.format(result, digits: 10)
      assert formatted =~ "9.842519685"
    end

    test "exponential option" do
      result = Unity.eval!("3 meters to feet")
      {:ok, formatted} = Unity.format(result, format: :terse, exponential: true)
      assert formatted =~ "E"
    end

    test "output_format option with printf syntax" do
      result = Unity.eval!("3 meters to feet")
      {:ok, formatted} = Unity.format(result, format: :terse, output_format: "%.8g")
      assert formatted == "9.8425197"
    end

    test "show_reciprocal option" do
      result = Unity.eval!("3 meters to feet")
      {:ok, formatted} = Unity.format(result, show_reciprocal: true)
      assert formatted =~ "feet"
      assert formatted =~ "/ "
    end
  ...
  describe "search" do
    test "Aliases.suggest finds similar names" do
      suggestions = Unity.Aliases.suggest("metrs")
  ...
  describe "aliases" do
    test "common abbreviations resolve" do
      assert {:ok, "meter"} = Unity.Aliases.resolve("m")
      assert {:ok, "kilometer"} = Unity.Aliases.resolve("km")
      assert {:ok, "foot"} = Unity.Aliases.resolve("ft")
      assert {:ok, "mile-per-hour"} = Unity.Aliases.resolve("mph")
      assert {:ok, "kilogram"} = Unity.Aliases.resolve("kg")
      assert {:ok, "liter"} = Unity.Aliases.resolve("L")
      assert {:ok, "celsius"} = Unity.Aliases.resolve("°C")
    end

    test "CLDR names pass through" do
      assert {:ok, "meter"} = Unity.Aliases.resolve("meter")
      assert {:ok, "kilogram"} = Unity.Aliases.resolve("kilogram")
      assert {:ok, "second"} = Unity.Aliases.resolve("second")
    end

    test "unknown unit returns error" do
      assert {:error, :unknown_unit} = Unity.Aliases.resolve("frobnicator")
    end
  ...
  describe "error formatting" do
    test "unknown unit error" do
      formatted = Unity.Error.format("unknown unit: \"frobnicator\"")
      assert formatted =~ "Unknown unit"
    end

    test "parse error" do
      formatted = Unity.Error.format("parse error: expected expression")
      assert formatted =~ "Parse error"
    end

    test "conformability error" do
      formatted = Unity.Error.format("cannot add: incompatible units")
